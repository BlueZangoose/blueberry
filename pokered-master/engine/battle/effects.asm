...
;~~~~~BOOST SIDE EFFECT BEGINS~~~~~
BoostSideEffect:
	jpfar BoostSideEffect_
	
BoostSideEffectFinish:
	ret
;~~~~~BOOST SIDE EFFECT ENDS~~~~~
;~~~~~MODIFIER SIDE EFFECT BEGINS~~~~~
ModifierSideEffect:
	jpfar ModifierSideEffect_
	
ModifierSideEffectFinish:
	ret
;~~~~~MODIFIER SIDE EFFECT ENDS~~~~~
;~~~~~CURE STATUS EFFECT BEGINS~~~~~
CureStatusEffect:
	jpfar CureStatusEffect_
	
CureStatusEffectFinish:
	ret
;~~~~~CURE STATUS EFFECT ENDS~~~~~
;~~~~~STATUS COUNTEREFFECT BEGINS~~~~~
StatusCounterEffect:
	jpfar StatusCounterEffect_
	
StatusCounterEffectFinish:
	ret
;~~~~~STATUS COUNTEREFFECT ENDS~~~~~
;~~~~~BURN EFFECT BEGINS~~~~~
BurnEffect:
	jpfar BurnEffect_
;~~~~~BURN EFFECT ENDS~~~~~
;~~~~~TYPE-CHANGE EFFECT BEGINS~~~~~
TypeChangeEffect:
	jpfar TypeChangeEffect_
;~~~~~TYPE-CHANGE EFFECT ENDS~~~~~
;~~~~~STATUS SIDE EFFECT BEGINS~~~~~
StatusSideEffect:
	xor a
	ld [wAnimationType], a ; resets animation type (vertical shake, horizontal shake etc) to 0. Don't know why, just seems standard.
	call CheckTargetSubstitute ; test bit 4 of d063/d068 flags [target has substitute flag]
	ret nz ; return if they have a substitute, can't effect them
	ldh a, [hWhoseTurn]	; 0 on player's turn, 1 on enemy's turn
	and a
	jp nz, .enemyStatusSideEffect	;enemy's turn
	jp .playerStatusSideEffect		;player's turn
.enemyStatusSideEffect
	;Check if the move matches enemy's type
	ld a, [wEnemyMoveType]
	ld b, a
	...
	ret z
	;Check finished. Load data
	ld hl, wBattleMonStatus
	ld de, wEnemyMoveEffect
	jp .testForCurrentStatus
.playerStatusSideEffect
	;Check if the move matches enemy's type
	...
	jp .testForCurrentStatus
.testForCurrentStatus
	ld a, [hl]
	and a
	ret nz	; return if status is anything other than 0
.statusSideEffect:
	ld a, [de]
;25% psn
	cp POISON_SIDE_EFFECT_25
	jp nz, :+
	ld b, 25 percent		;chance 25
	ld c, 1 << PSN			;poison
	jp .applyStatus
:;25% psn - acrid water
	cp ACRID_SIDE_EFFECT
	jp nz, :+
	ld b, 25 percent		;chance 25
	ld c, 1 << PSN			;poison
	jp .applyStatus
:;40% psn
	cp POISON_SIDE_EFFECT_40
	jp nz, :+
	ld b, 40 percent		;chance 40
	ld c, 1 << PSN			;poison
	jp .applyStatus
:;100% psn
	cp POISON_SIDE_EFFECT_100
	jp nz, :+
	ld b, 100 percent		;chance 100
	ld c, 1 << PSN			;poison
	jp .applyStatus
:;15% para
	cp PARALYZE_SIDE_EFFECT_15
	jp nz, :+
	ld b, 15 percent		;chance 15
	ld c, 1 << PAR			;paralyze
	jp .applyStatus
:;25% para
	cp PARALYZE_SIDE_EFFECT_25
	jp nz, :+
	ld b, 25 percent		;chance 25
	ld c, 1 << PAR			;paralyze
	jp .applyStatus
:;25% para - MEGAVOLT
	cp MEGAVOLT_SIDE_EFFECT
	jp nz, :+
	ld b, 25 percent		;chance 25
	ld c, 1 << PAR			;paralyze
	jp .applyStatus
:;100% para
	...
	jp .applyStatus
:;10% burn
	cp BURN_SIDE_EFFECT_10
	jp nz, :+
	ld b, 10 percent		;chance 10
	ld c, 1 << BRN			;burn
	jp .applyStatus
:;18% burn
	cp BURN_SIDE_EFFECT_18
	jp nz, :+
	ld b, 18 percent		;chance 18
	ld c, 1 << BRN			;burn
	jp .applyStatus
:;30% burn
	cp BURN_SIDE_EFFECT_30
	jp nz, :+
	ld b, 30 percent		;chance 30
	ld c, 1 << BRN			;burn
	jp .applyStatus
:;100% burn
	cp BURN_SIDE_EFFECT_100
	jp nz, :+
	ld b, 100 percent		;chance 100
	ld c, 1 << BRN			;burn
	jp .applyStatus
:;8% sleep
	cp SLEEP_SIDE_EFFECT_8
	jp nz, :++
	ld b, 8 percent			;chance 8
	push hl
:	;loop
	call BattleRandom
	and $7
	...
	jp .applyStatus
:;14% sleep
	cp SLEEP_SIDE_EFFECT_14
	jp nz, :++
	ld b, 14 percent		;chance 14
	push hl
:	;loop
	call BattleRandom
	and $7
	...
	jp .applyStatus
:;6% freeze
	cp FREEZE_SIDE_EFFECT_6
	jp nz, :+
	ld b, 6	 percent		;chance 6
	ld c, 1 << FRZ			;freeze
	;jp z, .freeze6
	jp .applyStatus
:;11% freeze
	cp FREEZE_SIDE_EFFECT_11
	jp nz, :+
	ld b, 11 percent		;chance 11
	ld c, 1 << FRZ			;freeze
	;jp z, .freeze11
	jp .applyStatus
:
.applyStatus
	...
	call Random
	pop hl
	cp b			; if Random is above the probability (b), skip applying the status.
	jp nc, .statusSideEffectFinished
	ld a, c
	ld [hl], a		; Apply the status!
;burn
	cp 1 << BRN			; 16
	jp nz, :+
...
:;paralysis				; 64
	...
	jp .statusSideEffectFinished
:;sleep
	;cp 7		; SLP is anything 7 or below
	;jp nc, :+
; Couldn't get these two lines to work. Not needed as sleep is the only option remaining. If you fix them, uncomment the : below.
	...
	jp .statusSideEffectFinished
;:
.statusSideEffectFinished
	ldh a, [hWhoseTurn]	; 0 on player's turn, 1 on enemy's turn
	and a
	jp nz, .enemyMove	;enemy's turn
	jp .playerMove		;player's turn
.enemyMove
	ld hl, wEnemyMoveEffect
	ld de, wEnemyBattleStatus2
	jp .checkSpecialMoves
.playerMove
	ld hl, wPlayerMoveEffect
	ld de, wPlayerBattleStatus2
	jp .checkSpecialMoves
.checkSpecialMoves				;If it's not a special move, return
	ld a, ACRID_SIDE_EFFECT
	cp [hl]
	jp z, .acridWater
	ld a, MEGAVOLT_SIDE_EFFECT
	cp [hl]
	jp z, .megavolt
	ret
.acridWater						;if it's acrid water, run it all again with a different side effect loaded
	ld a, PARALYZE_SIDE_EFFECT_25
	ld [hl], a
	jp StatusSideEffect
.megavolt						;if it's megavolt, set the recharge bit then return
	ld a, [de]
	set NEEDS_TO_RECHARGE, a
	ld [de], a
	ret
;~~~~~STATUS SIDE EFFECT ENDS~~~~~
...
;	cp b ; do target type 2 and move type match?
;	ret z  ; return if they match
;Check what effect the player's move has
;	ld a, [wPlayerMoveEffect]
;	cp PARALYZE_SIDE_EFFECT1 + 1
;	ld b, 10 percent + 1
;	jr c, .regular_effectiveness ; if the effect is lower on the list it's higher chance
; higher chance
;	ld b, 30 percent + 1
...
AttackHalvedDueToBurnText:
	text_far _AttackHalvedDueToBurnText
	text_end
...
PrintAttackHalvedDueToBurnText:
	ld hl, AttackHalvedDueToBurnText
	jp PrintText
...
.incrementStatMod
	...
	dec de				;	move effect -> move num
	ld a, [de]
	cp CHARGE_POUCH ; is it a +3 effect?
	jr nz, .checkModifier
	inc b ; if so, increment stat mod again
.checkModifier
	ld a, $d
	cp b ; unless it's already +6
	jr nc, .ok					;	no carry = b lower than a = stat mod lower than 14
	ld b, a
.ok
	ld [hl], b
	ld a, c
...
.playerTurn
	...
	jr c, .rose				; if attackdown1 is higher than the effect, jump
	dec de
	ld a, [de]
	cp CHARGE_POUCH
	jr nz, .rose
	ld hl, ThrobbinglyRoseText
	ret
.rose
	ld hl, RoseText
	ret
...
.setNumberOfHits
	...
	; start looking for moves by name
	ld hl, wPlayerSelectedMove
	ldh a, [hWhoseTurn]
	and a
	jr z, .checkMultihitMoves
...
.checkMultihitMoves
	...
	jr z, .twoToFourHits
	cp DRUMBEAK
	jr z, .threeToFourHits
	cp ECHO_SONG
	jr z, .threeToFourHits
	cp MUD_BULLETS
	jr z, .threeToFiveHits
	cp CURRENT_FLOW
	jr z, .currentFlow
	; falls through if it's a standard 2-5 hits move
.twoToFiveHits
	call BattleRandom
	and $3 			; removes all bits beyond the first two bits (converts the random number into 0, 1, 2 or 3)
	inc a
	inc a ; adds 2 more hits (total was 0-3, is now 2-5)
.saveNumberOfHits
	ld [de], a
	ld [bc], a
	ret
.oneToThreeHits
 	call BattleRandom
	and 3
	jr nz, .saveNumberOfHits ; if 1, 2, or 3, jump. If 0, continue
	inc a
	inc a
	inc a
	jr .saveNumberOfHits
...
.twoToFourHits
 	call BattleRandom
	and 3 ; is now 0, 1, 2, 3
	inc a
	inc a
	cp 5
	jr nz, .saveNumberOfHits
	ld a, 3
	jr .saveNumberOfHits
.threeToFourHits
	call BattleRandom
...
.currentFlow
	push hl
	call HyperBeamEffect ; uses a and hl
	pop hl
	jr .twoToFiveHits
...
.flinchSideEffect
	ld a, [de]
	cp FLINCH_SIDE_EFFECT_10
	ld b, 10 percent + 1
	jr z, .gotEffectChance
	cp FLINCH_SIDE_EFFECT_20
	ld b, 20 percent + 1
	jr z, .gotEffectChance
	cp FLINCH_SIDE_EFFECT_30
	ld b, 30 percent + 1
	jr z, .gotEffectChance
	cp FLINCH_SIDE_EFFECT_50
	ld b, 50 percent + 1
	jr z, .gotEffectChance
.gotEffectChance
	call BattleRandom
