...
INCLUDE "data/battle/high_priority_moves.asm"
INCLUDE "data/battle/low_priority_moves.asm"

MainInBattleLoop:	; XXX START HERE XXX
...
; the link battle enemy has switched mons
	...
	add hl, bc
	ld a, [hl]
	cp MONKEYAROUND ; a MIRROR MOVE check is missing, might lead to a desync in link battles
...
;d is player priority
;e is enemy priority

	ld a, [wPlayerSelectedMove]
	ld c, a
;player high priority check
	ld hl, HighPriorityMoves     	; data/battle/high_priority_moves.asm
.playerHighPriorityLoop
	ld a, [hli]                  	; read move from move table
	cp c                         	; does it match the move about to be used?
	jr z, .playerHighPriority    	; if so, the move about to be used is a high priority move
	inc a                        	; move on to the next move, FF terminates loop
	jr nz, .playerHighPriorityLoop	; loop unless a is 0
;player low priority check
	ld hl, LowPriorityMoves      	; data/battle/low_priority_moves.asm
.playerLowPriorityLoop
	ld a, [hli]                  	; read move from move table
	cp c                         	; does it match the move about to be used?
	jr z, .playerLowPriority    	; if so, the move about to be used is a low priority move
	inc a                        	; move on to the next move, FF terminates loop
	jr nz, .playerLowPriorityLoop	; loop unless a is 0
;player no priority
	jr .playerNoPriority	        ; continue as a normal move
.playerHighPriority
	ld d, 3
	jr .enemyPriority
.playerNoPriority
...
.enemyPriority
	...
	ld c, a
;enemy high priority check
	ld hl, HighPriorityMoves     	; high priority move table
.enemyHighPriorityLoop
	ld a, [hli]                  	; read move from move table
	cp c                         	; does it match the move about to be used?
	jr z, .enemyHighPriority    	; if so, the move about to be used is a high priority move
	inc a                        	; move on to the next move, FF terminates loop
	jr nz, .enemyHighPriorityLoop	; check the next move
;enemy low priority check
	ld hl, LowPriorityMoves      	; low priority move table
.enemyLowPriorityLoop
	ld a, [hli]                  	; read move from move table
	cp c                         	; does it match the move about to be used?
	jr z, .enemyLowPriority    		; if so, the move about to be used is a low priority move
	inc a                        	; move on to the next move, FF terminates loop
	jr nz, .enemyLowPriorityLoop	; check the next move
;enemy no priority
	jr .enemyNoPriority	        	; continue as a normal move
.enemyHighPriority
	ld e, 3
	jr .comparePriority
.enemyNoPriority
...
.comparePriority
	ld a, d
	cp e
	jr c, .enemyMovesFirst			; e > a
	jr z, .compareSpeed				; e = a
	jr nz, .playerMovesFirst		; anything else

;	ld a, [wPlayerSelectedMove] ; BOOKMARK: #[A2]
;	cp QUICK_ATTACK
;	jr z, .playerUsedHighPrio
;	jr nz, .playerDidNotUseHighPrio
;.playerUsedHighPrio
;	ld a, [wEnemySelectedMove]
;	cp QUICK_ATTACK
;	jr z, .compareSpeed  ; if both used Quick Attack
;	jp .playerMovesFirst ; if player used Quick Attack and enemy didn't
;.playerDidNotUseHighPrio
;	ld a, [wEnemySelectedMove]
;	cp QUICK_ATTACK
;	jp z, .enemyMovesFirst ; if enemy used Quick Attack and player didn't
;; all checks for HIGH PRIO are now complete
;	ld a, [wPlayerSelectedMove]
;	cp LEAF_TRAP
;	jr z, .playerUsedLowPrio
;	cp VORTEX
;	jr z, .playerUsedLowPrio
;	cp CLOSECIRCUIT
;	jr z, .playerUsedLowPrio
;	jr nz, .playerDidNotUseLowPrio
;.playerUsedLowPrio
;	ld a, [wEnemySelectedMove]
;	cp LEAF_TRAP
;	jr z, .compareSpeed
...
;~~~ all checks for LOW PRIO are now complete ~~~
.compareSpeed
	ld de, wBattleMonSpeed			; player speed value
	ld hl, wEnemyMonSpeed			; enemy speed value
	ld c, $2				; number of bytes needed to hold a speed value (1 byte caps out at 255)
	call StringCmp				; Sets z to 1 if equal, 0 if not. sets c to 1 if [hl] is larger.
	jr z, .speedEqual			; Jump to .speedEqual if speed is identical
	jr nc, .playerMovesFirst 		; jump to .playerMovesFirst if [hl] is not larger
	jr .enemyMovesFirst			; jump to .enemyMovesFirst if [hl] is not larger
...
.invertOutcome
	...
	jr .playerMovesFirst
.enemyMovesFirst	; ###### Bookmark #[A3]
	ld a, $1
	ldh [hWhoseTurn], a	; set hWhoseTurn to 1 (0 for players, 1 for enemies).
	callfar TrainerAI	; sets the carry flag if the AI uses an "action" (i.e. an item, or switching Pokémon)
	jr c, .AIActionUsedEnemyFirst ; Jump to using the action if the carry flag is set
...
.atLeastTwoMovesAvailable
	ld a, [wIsInBattle]	; check who the battle is against (1 for wild, 2 for trainer)
	dec a			; set z to 1 if wild battle
...
.playerHasNoSpecialCondition
	...
	jr nz, PlayerCanExecuteChargingMove		; when charging bit is set, we execute normal attack code
...
CheckIfPlayerNeedsToChargeUp:
	...
	cp CHARGE_EFFECT
	jp z, JumpMoveEffect					; when charging bit is not set, we jump to effects.asm to set it
	cp FLY_EFFECT
	jp z, JumpMoveEffect					; when charging bit is not set, we jump to effects.asm to set it
	cp CHARGE_PARALYZE_EFFECT
	jp z, JumpMoveEffect					; when charging bit is not set, we jump to effects.asm to set it
...
PlayerCanExecuteChargingMove:
	...
	res INVULNERABLE, [hl]
;To make charge moves have multiple effects, you must change its effect here (the only part exclusive to charge moves)
	ld a, [wPlayerMoveEffect]
	cp CHARGE_PARALYZE_EFFECT
	jp z, .chargeParalyzeEffect
	cp CHARGE_BOOST_EFFECT
	jp z, .chargeBoostEffect
	jp PlayerCanExecuteMove
.chargeParalyzeEffect
	ld a, PARALYZE_SIDE_EFFECT_100
...
.focusEnergyUsed
	ld b, $ff			; new - this results in a crit rate of 50% (it's halved further down)
	;srl b				; old bugged version
.noFocusEnergyUsed
...
.next
	cp CLOSECIRCUIT
	jr z, .isPhysicalCounter	; jump if using Counter
	cp VORTEX
	jr z, .isSpecialCounter		; jump if using Mirror Coat
	cp EMBERSKIN
	jr z, .isEffectCounter	
	cp STATIC_FLUFF
	jr z, .isEffectCounter	
	cp SLIME_COAT
	jr z, .isEffectCounter		
...
.next
	ld [hl], a
	cp MINDREADER ; did the target Pokemon last use Mirror Move, and miss?
...
; note that this means that even the highest accuracy is still just a 255/256 chance, not 100%
	...
	ret z 				; If so, we need not calculate
	call BattleRandom
	cp b
	jr nc, .moveMissed
	ret
...
; for more detailed commentary, see equivalent function for player side (ExecutePlayerMove) BKMRK #[A4]
ExecuteEnemyMove:
	ld a, [wEnemySelectedMove]
	inc a
